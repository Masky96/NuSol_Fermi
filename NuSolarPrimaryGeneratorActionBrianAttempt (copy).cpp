...
namespace User{
  ...
  */
  //The preceediing code was an attempt at reading in manual particle properties
  ...
  void NuSolarPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
  {
    ...
    //fireparticle = 666 means we are going for cosmic ray showers.
	...
	fparticleGun->GeneratePrimaryVertex(anEvent); 
      ...
      //Fires particles based on the CRY output shower.out
      */
  



 float ShowerArray[10000][10];
    std::ifstream file("shower10000.out");
    ...
    //std::vector<std::pair<std::string, std::vector<double>>> ShowerArray = read_csv("three_cols.csv");
	...
	int j = int round( G4UniformRand() * 10000 );
