...
void LXeSteppingAction::UserSteppingAction(const G4Step* theStep)
{
  ...
  //New
  //G4ThreeVector deltM         = theStep->GetDeltaMomentum();
  G4ThreeVector deltM         = theStep->GetPreStepPoint()->GetMomentum();
  
  G4double magRat             = deltM.getZ()/(sqrt(deltM.getX()*deltM.getX() + deltM.getY()*deltM.getY() +  deltM.getZ()*deltM.getZ()));
