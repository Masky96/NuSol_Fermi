...
{
  G4cout << "Primary Generator Action Starts.\n";
...
{  
  //G4cout << "GeneratePrimaries starts!\n";
  ...
  // particle and ion tables for use
    ...
    //energyPri = 0.;
    //G4cout << "Before copy\n";
    std::vector< std::vector< User::event* >* > showerCopy = *(CSatRunAction::showers);
    
    // find the number of events so we can loop around if beamOn is too big
    G4int modulus = showerCopy.size();
    
    G4cout << "The size of showers is " << modulus << ".\n";
    
    //while(!showerShackle->try_lock());
    showerShackle.lock();
    std::vector<User::event*> *theShowerCopy;
    {
      //std::lock_guard<std::mutex> lock(showerShackle);
      theShowerCopy = new std::vector<User::event*>(
						    ((showerCopy))[nShowers % modulus] -> begin(),
						    ((showerCopy))[nShowers % modulus] -> end()
						    );// make a copy of the relevant shower
      nShowers++;// ready for next iteration
    }
    showerShackle.unlock();
    
    G4cout << "The shower has " << theShowerCopy->size() << " particles.\n";
    
    User::event *thisParticle = new User::event;
...
G4ThreeVector TranslationToHit(User::event){
  G4ThreeVector translator (0,0,0); //initialize to zero

  {// will be in a while loop
  }
  return translator;
}
...
G4int CSatPrimaryGeneratorAction::nShowers = 0;
//G4int *CSatPrimaryGeneratorAction::nShowers = new G4int(0);

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......


std::mutex CSatPrimaryGeneratorAction::showerShackle;
//std::mutex *CSatPrimaryGeneratorAction::showerShackle = new std::mutex;
