...
G4bool CSatGPMTSD::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
  ...
  if (G4UniformRand() < quanEff->Value(wavelength))
    {
      //G4AnalysisManager::Instance()->FillNtupleFColumn(evtID , 0 , timeG);
      //G4AnalysisManager::Instance()->FillNtupleFColumn(evtID , 1 , wavelength);
      //G4AnalysisManager::Instance()->AddNtupleRow(evtID);
      ...
      if (evtID == 2)
	{
      ...
      */
	  ...
	  G4AnalysisManager::Instance()->FillNtupleFColumn(evtID+3 , 0 , timeG);
	  //G4AnalysisManager::Instance()->FillNtupleFColumn(evtID+3 , 1 , wavelength);
	  G4AnalysisManager::Instance()->AddNtupleRow(evtID+3);
